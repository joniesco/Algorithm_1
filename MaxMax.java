...
public class MaxMax {
    ...
    public static int maxMaxM2Recursive(int[] arr) {
        ...
        for (int i = 0; i < arr.length; i++) {
            nodes[i] = new Node1(arr[i]);
        }
        Node1 max1 = maxMaxM2Recursive(nodes,0,nodes.length-1,comparisons);
    ...
    private static Node1 maxMaxM2Recursive(Node1[] nodes, int low, int high,int[] comparisons) {
        if(high == low) return nodes[low];
        int mid = (low + high)/2;
        Node1 maxL = maxMaxM2Recursive(nodes,low,mid,comparisons);
        Node1 maxR = maxMaxM2Recursive(nodes,mid+1,high,comparisons);
        comparisons[0]++;
    ...
    public static int maxMaxM2Induction(int[] arr) {
        ...
        for (int i = 0; i < arr.length; i++) {
            list.add(new Node1(arr[i]));
        }
        int i = 0;
        while(list.size() > 1) {
            Node1 x = list.get(i);
            Node1 y = list.get(i+1);
            comparisons++;
